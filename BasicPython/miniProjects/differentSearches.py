...
def exhaustiveEnumeration(obj):
    ...
    Here you want to check an objective, that is find the square root using multiplication.
    """
    answer = 0
    while answer ** 2 == obj:
        answer += 1
...
def BinarySearch(obj):
    """
    Efficient method for searching results while it cuts the range of iteration by two.
...
def main():
    while(True):
        ...
        obj = float(input("Which number are you searching for? The square root of: "))
        clearScreen()
        if value == 1:
