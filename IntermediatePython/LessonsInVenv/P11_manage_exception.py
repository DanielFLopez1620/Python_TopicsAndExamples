...
def main():
    ...
    * try:
        [Process]
    --> With a try, you can execute code that is running in a protected way, if
    an exception is called, it is 'encapsulated' and launched but it will be 
    caught by the 'catch statmente'.
    ...
    * catch <Exception> as <ExpName>:
    ...
    ZeroDivisonError, it will not be caught by the IndexError block. Also, if the
    name of the exception is long, you can make an alias for it, like 'as FNF' in 
    the case of FileNotFoundException.

    * catch Exception:
        [Plan Z]
    --> If you want to prevent every type of exception, these is the father block
    to achieve it. Just make sure it is at the end of the 'catch' statments. But
    note that it is not recommended to manage general exceptions.
    
    * raise <Exception>("Message/Traceback")
    --> You can also launch exception manually, because sometimes you know that 
    somthing is an error, but Python doesn't. An example? You want to process data, but
    it doesn't work if the data that they send you is only a zero.
    
    * finally:
        [Do not forget]
    --> It is the last statment that runs, it does something whether an exception ocurred
    or everything went write. It is useful to close files or disconnect from databases.
    ...
    * assert <condition>, <Message error>
    ...
    while(follow):
        ...
        try:
            ...
            print("This is a loop code for palindrome and division...\nDIVISION:")
            ...
            print("Something unexpected happened, see and analyze the exception.")
        finally:
