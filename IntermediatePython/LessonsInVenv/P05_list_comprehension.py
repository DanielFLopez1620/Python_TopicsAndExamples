...
def my_list_comp(limit):
    ...
    """
    result = [i * 4 for i in range(0, limit) if not (i * 4 % 6)
              and not (i * 4 % 9)]
    return result
...
def main():
    ...
    print(
        """
        There are many ways to assign and create variables, you know,for
        example:
        ...
        It is called list comprehension, you can use it while creating a
        single line description loop (and optional conditions), let's see
        its structure...
        <list> = [i for i in range(<range>) if <condition>]

        Throughout this program you will see more about all the mentioned ways
        to create listed above:
