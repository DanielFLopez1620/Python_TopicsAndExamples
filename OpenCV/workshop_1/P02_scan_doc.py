...
def getWarpPersp(image, contour, width, height, cropPix):
    ...
    imgCrop = imgWrap[20: imgWrap.shape[0] - cropPix,
                      20: imgWrap.shape[1] - cropPix]
...
def main():
    ...
    while True:
        ...
        if docCnt.size != 0:
            ...
            results = stackImages(
                0.6, [[fotogram, imageCnt], [imageThreshold, docPerps]]
            )
        else:
            results = stackImages(0.6, [[fotogram, fotogram],
                                        [fotogram, fotogram]])
